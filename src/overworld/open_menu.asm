
OPEN_MENU_BUTTON:
	BEGIN_C_FUNCTION_FAR
	STACK_RESERVE_VARS
	STACK_RESERVE_INT32
	STACK_RESERVE_INT32
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT8
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT32
	STACK_RESERVE_INT16
	END_STACK_VARS
	JSL FREEZE_ENTITIES
	LDA #SFX::CURSOR1
	JSL PLAY_SOUND
	CREATE_WINDOW_NEAR #WINDOW::UNKNOWN00
	SEP #PROC_FLAGS::ACCUM8
	STZ SKIP_ADDING_COMMAND_TEXT
	JSR POPULATE_COMMAND_MENU
	SEP #PROC_FLAGS::ACCUM8
	STZ RESTORE_MENU_BACKUP
@MAIN_PAUSE_MENU:
	REP #PROC_FLAGS::ACCUM8
	LDA #0
	JSR SET_WINDOW_FOCUS
	LDA #1
	JSR SELECTION_MENU
	STORE_INT1632 @VIRTUAL06
	LDA @VIRTUAL06
	CMP #MENU_OPTIONS::TALK_TO
	BEQ @TALK_TO
	CMP #MENU_OPTIONS::GOODS
	BEQ @GOODS
	CMP #MENU_OPTIONS::PSI
	BEQL @PSI
	CMP #MENU_OPTIONS::EQUIP
	BEQL @EQUIP
	CMP #MENU_OPTIONS::CHECK
	BEQL @CHECK
	CMP #MENU_OPTIONS::STATUS
	BEQL @STATUS
	JMP @UNKNOWN75
@TALK_TO:
	JSL TALK_TO
	MOVE_INT_CONSTANT NULL, @VIRTUAL0A
	CMP32 @VIRTUAL06, @VIRTUAL0A
	BNE @UNKNOWN7
	LOADPTR MSG_SYS_HANASU_NG, @VIRTUAL06
@UNKNOWN7:
	MOVE_INT @VIRTUAL06, @LOCAL00
	JSL DISPLAY_TEXT
	JMP @UNKNOWN75
@GOODS:
	JSR OPEN_HP_AND_WALLET
@UNKNOWN9:
	LDA GAME_STATE+game_state::player_controlled_party_count
	AND #$00FF
	CMP #1
	BNE @GOODS_MANY_PARTY_MEMBERS
	LDY #.LOWORD(GAME_STATE) + game_state::party_members
	STY @LOCAL08
	LDX #1
	LDA __BSS_START__,Y
	AND #$00FF
	JSL GET_CHARACTER_ITEM
	CMP #0
	BEQL @MAIN_PAUSE_MENU
	LDX #2
	LDY @LOCAL08
	LDA __BSS_START__,Y
	AND #$00FF
	JSR INVENTORY_GET_ITEM_NAME
	LDY @LOCAL08
	SEP #PROC_FLAGS::ACCUM8
	LDA __BSS_START__,Y
	STORE_INT832 @VIRTUAL06
	REP #PROC_FLAGS::ACCUM8
	MOVE_INT @VIRTUAL06, @LOCAL07
	LDA #0
	JSL SWAP_RAISED_HP_PP_WINDOW
	BRA @UNKNOWN12
@GOODS_MANY_PARTY_MEMBERS:
	LDA #0
	JSR OPEN_EQUIP_SELECT_MENU
	LOADPTR CREATE_INVENTORY_WINDOW, @LOCAL00
	MOVE_INT_CONSTANT NULL, @LOCAL01
	LDX #1
	LDA #0
	JSR CHAR_SELECT_PROMPT
	STORE_INT1632 @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL07
@UNKNOWN12:
	MOVE_INT_CONSTANT NULL, @VIRTUAL0A
	CMP32 @VIRTUAL06, @VIRTUAL0A
	BNE @UNKNOWN14
	LDA #WINDOW::INVENTORY
	JSL CLOSE_WINDOW
	JSR CLOSE_EQUIP_SELECT_MENU
	JMP @MAIN_PAUSE_MENU
@UNKNOWN14:
	LDX #1
	LDA @VIRTUAL06
	JSL GET_CHARACTER_ITEM
	CMP #0
	BEQL @UNKNOWN9
@UNKNOWN15:
	LDA #1
	JSR OPEN_EQUIP_SELECT_MENU
	LDA #WINDOW::INVENTORY
	JSR SET_WINDOW_FOCUS
	LDA #1
	JSR SELECTION_MENU
	STA @VIRTUAL04
	STA @LOCAL06
	JSL BACKUP_MENU_SELECTION
	JSR CLOSE_EQUIP_SELECT_MENU
	LDA @VIRTUAL04
	BNE @GOODS_ITEM_SELECTED
	LDA GAME_STATE+game_state::player_controlled_party_count
	AND #$00FF
	CMP #1
	BNEL @UNKNOWN9
	LDX #1
	LDA GAME_STATE + game_state::party_members
	AND #$00FF
	JSL GET_CHARACTER_ITEM
	CMP #0
	BEQ @UNKNOWN17
	LDA #SFX::MENU_OPEN_CLOSE
	JSL PLAY_SOUND
	JSL RESET_ACTIVE_PARTY_MEMBER_HP_PP_WINDOW
@UNKNOWN17:
	LDA #WINDOW::INVENTORY
	JSL CLOSE_WINDOW
	JMP @MAIN_PAUSE_MENU
@GOODS_ITEM_SELECTED:
	CREATE_WINDOW_NEAR #WINDOW::INVENTORY_MENU
	MOVE_INT @LOCAL07, @VIRTUAL06
	LDA @VIRTUAL06
	DEC
	LDY #.SIZEOF(char_struct)
	JSL MULT168
	TAX
	LDA PARTY_CHARACTERS + char_struct::afflictions + STATUS_GROUP::PERSISTENT_EASYHEAL,X
	AND #$00FF
	TAX
	BEQ @GOODS_ITEM_SELECTED_ALIVE
	STX @VIRTUAL02
	LDA #4
	CLC
	SBC @VIRTUAL02
	BRANCHLTEQS @GOODS_ITEM_SELECTED_ALIVE
	LDX #1
	BRA @UNKNOWN22
@GOODS_ITEM_SELECTED_ALIVE:
	LDX #$0000
@UNKNOWN22:
	TXY
	STY @LOCAL08
	TYX
	LDA #0
	JSL MOVE_CURRENT_TEXT_CURSOR
	BRA @UNKNOWN24
@UNKNOWN23:
	TYX
	INX
	STX @LOCAL05
	LOADPTR ITEM_USE_MENU_STRINGS, @VIRTUAL06
	TYA
	OPTIMIZED_MULT @VIRTUAL04, 6
	CLC
	ADC @VIRTUAL06
	STA @VIRTUAL06
	STA @LOCAL00
	LDA @VIRTUAL06+2
	STA @LOCAL00+2
	MOVE_INT_CONSTANT NULL, @LOCAL01
	TXA
	JSR CREATE_NEW_MENU_OPTION_WITH_USER_DATA
	LDX @LOCAL05
	TXY
	STY @LOCAL08
@UNKNOWN24:
	LDY @LOCAL08
	CPY #4
	BCC @UNKNOWN23
	LDY #0
	TYX
	LDA #1
	JSL CREATE_MENU_OPTION_TABLE
	LDA #0
	STA @VIRTUAL02
@UNKNOWN25:
	REP #PROC_FLAGS::ACCUM8
	LDA @VIRTUAL02
	BEQ @UNKNOWN26
	LDA #WINDOW::INVENTORY
	JSR SET_WINDOW_FOCUS
	SEP #PROC_FLAGS::ACCUM8
	LDA @LOCAL04
	STA @VIRTUAL00
	REP #PROC_FLAGS::ACCUM8
	LDA @VIRTUAL00
	AND #$00FF
	BEQ @UNKNOWN27
	JSR PRINT_MENU_ITEMS
	BRA @UNKNOWN27
@UNKNOWN26:
	LDA #WINDOW::INVENTORY_MENU
	JSR SET_WINDOW_FOCUS
	JSR PRINT_MENU_ITEMS
@UNKNOWN27:
	LDA #WINDOW::INVENTORY_MENU
	JSR SET_WINDOW_FOCUS
	LDA #1
	JSR SELECTION_MENU
	STORE_INT1632 @VIRTUAL0A
	LDA @VIRTUAL0A
	BEQ @UNKNOWN30
	CMP #1
	BEQ @GOODS_ITEM_USE
	CMP #4
	BEQ @GOODS_ITEM_HELP
	CMP #2
	BEQL @UNKNOWN34
	CMP #3
	BEQL @UNKNOWN63
	JMP @UNKNOWN75
@UNKNOWN30:
	JSR CLOSE_FOCUS_WINDOW
	LDA #WINDOW::INVENTORY
	JSR SET_WINDOW_FOCUS
	JMP @UNKNOWN15
@GOODS_ITEM_USE:
	LDA #1
	STA @VIRTUAL02
	LDY #0
	LDA @LOCAL06
	STA @VIRTUAL04
	LDX @VIRTUAL04
	MOVE_INT @LOCAL07, @VIRTUAL06
	LDA @VIRTUAL06
	JSR OVERWORLD_USE_ITEM
	CMP #0
	BNEL @UNKNOWN75
	SEP #PROC_FLAGS::ACCUM8
	LDA #0
	STA @VIRTUAL00
	STA @LOCAL04
	JMP @UNKNOWN25
@GOODS_ITEM_HELP:
	.A16
	LDA #0
	JSR CLEAR_WINDOW
	LDA #2
	JSR CLEAR_WINDOW
	SEP #PROC_FLAGS::ACCUM8
	LDA #$00FF
	STA RESTORE_MENU_BACKUP
	REP #PROC_FLAGS::ACCUM8
	CREATE_WINDOW_NEAR #WINDOW::TEXT_STANDARD
	MOVE_INT @LOCAL07, @VIRTUAL06
	LDA @VIRTUAL06
	TAY
	STY @LOCAL08
	LDA @LOCAL06
	STA @VIRTUAL04
	LDX @VIRTUAL04
	TYA
	JSL GET_CHARACTER_ITEM
	STA @LOCAL06
	LOADPTR ITEM_CONFIGURATION_TABLE, @VIRTUAL0A
	LDA @LOCAL06
	LDY #.SIZEOF(item)
	JSL MULT168
	CLC
	ADC #item::help_text
	CLC
	ADC @VIRTUAL0A
	STA @VIRTUAL0A
	DEREFERENCE_PTR_TO @VIRTUAL0A, @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL00
	JSL DISPLAY_TEXT
	LDA #WINDOW::TEXT_STANDARD
	JSL CLOSE_WINDOW
	LDA #WINDOW::UNKNOWN00
	JSR SET_WINDOW_FOCUS
	SEP #PROC_FLAGS::ACCUM8
	LDA #1
	STA SKIP_ADDING_COMMAND_TEXT
	JSR POPULATE_COMMAND_MENU
	.A16
	LDX #2
	LDY @LOCAL08
	TYA
	JSR INVENTORY_GET_ITEM_NAME
	LDA #WINDOW::INVENTORY_MENU
	JSL CLOSE_WINDOW
	LDA #WINDOW::INVENTORY
	JSR SET_WINDOW_FOCUS
	JMP @UNKNOWN15
@UNKNOWN34:
	LDA #WINDOW::INVENTORY
	JSR SET_WINDOW_FOCUS
	JSR CLEAR_FOCUS_WINDOW
	LDA #1
	STA @VIRTUAL02
	LDA #3
	JSR OPEN_EQUIP_SELECT_MENU
	LOADPTR CREATE_ITEM_TARGETTING_WINDOW, @LOCAL00
	MOVE_INT_CONSTANT NULL, @LOCAL01
	LDX #1
	LDA #2
	JSR CHAR_SELECT_PROMPT
	STA @LOCAL03
	JSR CLOSE_EQUIP_SELECT_MENU
	LDA #WINDOW::UNKNOWN2C
	JSL CLOSE_WINDOW
	LDA @LOCAL03
	BNE @UNKNOWN35
	SEP #PROC_FLAGS::ACCUM8
	LDA #1
	STA @VIRTUAL00
	STA @LOCAL04
	JMP @UNKNOWN25
@UNKNOWN35:
	MOVE_INT @LOCAL07, @VIRTUAL06
	MOVE_INT1632 @LOCAL03, @VIRTUAL0A
	CMP32 @VIRTUAL0A, @VIRTUAL06
	BEQ @UNKNOWN37
	LDA @LOCAL06
	STA @VIRTUAL04
	LDX @VIRTUAL04
	LDA @VIRTUAL06
	JSL GET_CHARACTER_ITEM
	LDY #.SIZEOF(item)
	JSL MULT168
	.A16
	CLC
	ADC #item::flags
	TAX
	LDA f:ITEM_CONFIGURATION_TABLE,X
	AND #$00FF
	AND #ITEM_FLAGS::CANNOT_GIVE
	BEQ @UNKNOWN37
	CREATE_WINDOW_NEAR #WINDOW::TEXT_STANDARD
	MOVE_INT @VIRTUAL06, @LOCAL00
	JSR SET_WORKING_MEMORY
	MOVE_INT1632 @VIRTUAL04, @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL00
	JSR SET_ARGUMENT_MEMORY
	DISPLAY_TEXT_PTR MSG_SYS_GOODS_NOCARRY
	LDA #WINDOW::TEXT_STANDARD
	JSL CLOSE_WINDOW
	SEP #PROC_FLAGS::ACCUM8
	LDA #1
	STA @VIRTUAL00
	STA @LOCAL04
	JMP @UNKNOWN25
@UNKNOWN37:
	LDX #0
	STX @LOCAL02
	LDA @VIRTUAL06
	DEC
	LDY #.SIZEOF(char_struct)
	JSL MULT168
	.A16
	TAX
	LDA PARTY_CHARACTERS + char_struct::afflictions + STATUS_GROUP::PERSISTENT_EASYHEAL,X
	AND #$00FF
	TAY
	CPY #STATUS_0::UNCONSCIOUS
	BEQ @UNKNOWN38
	CPY #STATUS_0::DIAMONDIZED
	BNE @UNKNOWN39
@UNKNOWN38:
	LDX #5
	STX @LOCAL02
@UNKNOWN39:
	MOVE_INT1632 @LOCAL03, @VIRTUAL0A
	CMP32 @VIRTUAL0A, @VIRTUAL06
	.A16
	BEQ @UNKNOWN43
	LDX @LOCAL02
	INX
	STX @LOCAL02
	LDA @LOCAL03
	JSL FIND_INVENTORY_SPACE2
	CMP #0
	BEQ @UNKNOWN41
	LDX @LOCAL02
	INX
	INX
	STX @LOCAL02
@UNKNOWN41:
	LDA @LOCAL03
	DEC
	LDY #.SIZEOF(char_struct)
	JSL MULT168
	TAX
	LDA PARTY_CHARACTERS + char_struct::afflictions + STATUS_GROUP::PERSISTENT_EASYHEAL,X
	AND #$00FF
	TAY
	CPY #STATUS_0::UNCONSCIOUS
	BEQ @UNKNOWN42
	CPY #STATUS_0::DIAMONDIZED
	BNE @UNKNOWN43
@UNKNOWN42:
	LDX @LOCAL02
	INX
	STX @LOCAL02
@UNKNOWN43:
	CREATE_WINDOW_NEAR #WINDOW::TEXT_STANDARD
	JSR GET_ACTIVE_WINDOW_ADDRESS
	STA @LOCAL08
	CLC
	ADC #window_stats::working_memory
	TAY
	MOVE_INT_YPTRDEST @VIRTUAL06, __BSS_START__
	MOVE_INT1632 @LOCAL03, @VIRTUAL0A
	LDA @LOCAL08
	CLC
	ADC #window_stats::working_memory_storage
	TAY
	MOVE_INT_YPTRDEST @VIRTUAL0A, __BSS_START__
	LDA @LOCAL06
	STA @VIRTUAL04
	STORE_INT1632 @VIRTUAL0A
	LDA @LOCAL08
	CLC
	ADC #window_stats::argument_memory
	TAY
	MOVE_INT_YPTRDEST @VIRTUAL0A, __BSS_START__
	LDX @LOCAL02
	TXA
	BEQ @GOODS_GIVE_SELF_ALIVE_TEXT
	CMP #1
	BEQ @GOODS_GIVE_ALIVE_TO_ALIVE_FAIL_TEXT
	CMP #2
	BEQ @GOODS_GIVE_ALIVE_TO_DEAD_FAIL_TEXT
	CMP #3
	BEQL @GOODS_GIVE_ALIVE_TO_ALIVE_SUCC_TEXT
	CMP #4
	BEQL @GOODS_GIVE_ALIVE_TO_DEAD_SUCC_TEXT
	CMP #5
	BEQL @GOODS_GIVE_SELF_DEAD_TEXT
	CMP #6
	BEQL @GOODS_GIVE_DEAD_TO_ALIVE_FAIL_TEXT
	CMP #7
	BEQL @GOODS_GIVE_DEAD_TO_DEAD_FAIL_TEXT
	CMP #8
	BEQL @GOODS_GIVE_DEAD_TO_ALIVE_SUCC_TEXT
	CMP #9
	BEQL @GOODS_GIVE_DEAD_TO_DEAD_SUCC_TEXT
	JMP @GOODS_GIVE_INVALID_TEXT
@GOODS_GIVE_SELF_ALIVE_TEXT:
	DISPLAY_TEXT_PTR MSG_SYS_CARRY_SELF_ALIVE
	LDY @VIRTUAL04
	MOVE_INT @LOCAL07, @VIRTUAL06
	LDA @VIRTUAL06
	TAX
	LDA @LOCAL03
	JSL MOVE_ITEM_TO_PARTY_MEMBER
	JMP @UNKNOWN62
@GOODS_GIVE_ALIVE_TO_ALIVE_FAIL_TEXT:
	DISPLAY_TEXT_PTR MSG_SYS_CARRY_FAIL_OTHER_ALIVE_ALIVE
	JMP @UNKNOWN62
@GOODS_GIVE_ALIVE_TO_DEAD_FAIL_TEXT:
	DISPLAY_TEXT_PTR MSG_SYS_CARRY_FAIL_OTHER_ALIVE_DEAD
	JMP @UNKNOWN62
@GOODS_GIVE_ALIVE_TO_ALIVE_SUCC_TEXT:
	DISPLAY_TEXT_PTR MSG_SYS_CARRY_OTHER_ALIVE_ALIVE
	LDY @VIRTUAL04
	MOVE_INT @LOCAL07, @VIRTUAL06
	LDA @VIRTUAL06
	TAX
	LDA @LOCAL03
	JSL MOVE_ITEM_TO_PARTY_MEMBER
	JMP @UNKNOWN62
@GOODS_GIVE_ALIVE_TO_DEAD_SUCC_TEXT:
	DISPLAY_TEXT_PTR MSG_SYS_CARRY_OTHER_ALIVE_DEAD
	LDY @VIRTUAL04
	MOVE_INT @LOCAL07, @VIRTUAL06
	LDA @VIRTUAL06
	TAX
	LDA @LOCAL03
	JSL MOVE_ITEM_TO_PARTY_MEMBER
	JMP @UNKNOWN62
@GOODS_GIVE_SELF_DEAD_TEXT:
	DISPLAY_TEXT_PTR MSG_SYS_CARRY_SELF_DEAD
	LDY @VIRTUAL04
	MOVE_INT @LOCAL07, @VIRTUAL06
	LDA @VIRTUAL06
	TAX
	LDA @LOCAL03
	JSL MOVE_ITEM_TO_PARTY_MEMBER
	BRA @UNKNOWN62
@GOODS_GIVE_DEAD_TO_ALIVE_FAIL_TEXT:
	DISPLAY_TEXT_PTR MSG_SYS_CARRY_FAIL_OTHER_DEAD_ALIVE
	BRA @UNKNOWN62
@GOODS_GIVE_DEAD_TO_DEAD_FAIL_TEXT:
	DISPLAY_TEXT_PTR MSG_SYS_CARRY_FAIL_OTHER_DEAD_DEAD
	BRA @UNKNOWN62
@GOODS_GIVE_DEAD_TO_ALIVE_SUCC_TEXT:
	DISPLAY_TEXT_PTR MSG_SYS_CARRY_OTHER_DEAD_ALIVE
	LDY @VIRTUAL04
	MOVE_INT @LOCAL07, @VIRTUAL06
	LDA @VIRTUAL06
	TAX
	LDA @LOCAL03
	JSL MOVE_ITEM_TO_PARTY_MEMBER
	BRA @UNKNOWN62
@GOODS_GIVE_DEAD_TO_DEAD_SUCC_TEXT:
	DISPLAY_TEXT_PTR MSG_SYS_CARRY_OTHER_DEAD_DEAD
	LDY @VIRTUAL04
	MOVE_INT @LOCAL07, @VIRTUAL06
	LDA @VIRTUAL06
	TAX
	LDA @LOCAL03
	JSL MOVE_ITEM_TO_PARTY_MEMBER
	BRA @UNKNOWN62
@GOODS_GIVE_INVALID_TEXT:
	BRA @GOODS_GIVE_INVALID_TEXT
@UNKNOWN62:
	LDA #WINDOW::TEXT_STANDARD
	JSL CLOSE_WINDOW
	LDA #WINDOW::INVENTORY_MENU
	JSL CLOSE_WINDOW
	LDA #WINDOW::INVENTORY
	JSL CLOSE_WINDOW
	JMP @MAIN_PAUSE_MENU
@UNKNOWN63:
	CREATE_WINDOW_NEAR #WINDOW::TEXT_STANDARD
	MOVE_INT @LOCAL07, @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL00
	JSR SET_WORKING_MEMORY
	LDA @LOCAL06
	STA @VIRTUAL04
	STORE_INT1632 @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL00
	JSR SET_ARGUMENT_MEMORY
	DISPLAY_TEXT_PTR MSG_SYS_GOODS_DROP
	LDA #WINDOW::TEXT_STANDARD
	JSL CLOSE_WINDOW
	LDA #WINDOW::INVENTORY_MENU
	JSL CLOSE_WINDOW
	LDA #WINDOW::INVENTORY
	JSL CLOSE_WINDOW
	JMP @MAIN_PAUSE_MENU
@PSI:
	JSR OPEN_HP_AND_WALLET
	JSR GET_FIRST_PARTY_MEMBER_WITH_PSI
	STORE_INT1632 @VIRTUAL06
	MOVE_INT_CONSTANT NULL, @VIRTUAL0A
	CMP32 @VIRTUAL06, @VIRTUAL0A
	BEQ @UNKNOWN66
	LDA @VIRTUAL06
	DEC
	JSL SWAP_RAISED_HP_PP_WINDOW
@UNKNOWN66:
	JSR OVERWORLD_PSI_MENU
	CMP #0
	BNEL @UNKNOWN75
	JSR GET_PARTY_MEMBER_COUNT_WITH_PSI
	CMP #1
	BNEL @MAIN_PAUSE_MENU
	LDA #SFX::MENU_OPEN_CLOSE
	JSL PLAY_SOUND
	JSL RESET_ACTIVE_PARTY_MEMBER_HP_PP_WINDOW
	JMP @MAIN_PAUSE_MENU
@EQUIP:
	JSR OPEN_HP_AND_WALLET
	JSR UNKNOWN_C1AA5D
	LDA GAME_STATE+game_state::player_controlled_party_count
	AND #$00FF
	CMP #1
	BNEL @MAIN_PAUSE_MENU
	LDA #SFX::MENU_OPEN_CLOSE
	JSL PLAY_SOUND
	JSL RESET_ACTIVE_PARTY_MEMBER_HP_PP_WINDOW
	JMP @MAIN_PAUSE_MENU
@CHECK:
	JSL CHECK
	MOVE_INT_CONSTANT NULL, @VIRTUAL0A
	CMP32 @VIRTUAL06, @VIRTUAL0A
	BNE @UNKNOWN73
	LOADPTR MSG_SYS_NOPROBLEM, @VIRTUAL06
@UNKNOWN73:
	MOVE_INT @VIRTUAL06, @LOCAL00
	JSL DISPLAY_TEXT
	BRA @UNKNOWN75
@STATUS:
	JSR OPEN_HP_AND_WALLET
	SEP #PROC_FLAGS::ACCUM8
	LDA #1
	STA FORCE_LEFT_TEXT_ALIGNMENT
	JSR UNKNOWN_C1BB71
	SEP #PROC_FLAGS::ACCUM8
	STZ FORCE_LEFT_TEXT_ALIGNMENT
	JMP @MAIN_PAUSE_MENU
@UNKNOWN75:
	JSL CLEAR_INSTANT_PRINTING
	JSR HIDE_HPPP_WINDOWS
	JSR CLOSE_ALL_WINDOWS
@UNKNOWN76:
	JSL WINDOW_TICK
	.A16
	LDA ENTITY_FADE_ENTITY
	CMP #.LOWORD(-1)
	BNE @UNKNOWN76
	JSL UNFREEZE_ENTITIES
	END_C_FUNCTION
